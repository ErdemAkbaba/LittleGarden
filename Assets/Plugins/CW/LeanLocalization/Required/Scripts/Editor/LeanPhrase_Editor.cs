...
namespace Lean.Localization.Editor
{
	...
	public class LeanPhrase_Editor : CwEditor
	{
		...
		protected override void OnInspector()
		{
			...
			if (GUILayout.Button("All Auto Translate"))
			{
                foreach (var languageName in languageNames)
				{
                    var entry = default(LeanPhrase.Entry);
                    if (tgt.TryFindTranslation(languageName, ref entry) == false)                    
                    {
                        EditorGUILayout.BeginHorizontal();						
                        tgt.AddEntry(languageName);
                        DirtyAndUpdate();
				...
				{
					if(otherEntry.Language != "English")
					{
                        var languageInput = default(LeanLanguage);
                        var languageOutput = default(LeanLanguage);

						if (LeanLocalization.CurrentLanguages.TryGetValue(otherEntry.Language, out languageInput) == true && LeanLocalization.CurrentLanguages.TryGetValue(tgt.Entries[0].Language, out languageOutput) == true)
						{
							var languageCodeInput = languageInput.TranslationCode;
							var languageCodeOutput = languageOutput.TranslationCode;

                            Debug.Log($"Bos -> {languageCodeInput} -> {languageCodeOutput}");

                            var textOutput = default(string);
                            if (TryAutoTranslate(languageCodeOutput, languageCodeInput, tgt.Entries[0].Text, ref textOutput) == true)
                            {
